...
class FolderTileDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter: QPainter, option, index):
        ...
        bg = QColor(color)
        bg.setAlpha(28)
        active = index.data(PATH_ROLE) == self.active_path
        pen = QPen(color)
        pen.setWidth(4 if active else 1)
        painter.setPen(pen)
        painter.setBrush(bg)
        painter.drawRoundedRect(rect.adjusted(1, 1, -1, -1) if active else rect, 8, 8)
...
class FolderTileList(QListWidget):
    ...
    def set_active_path(self, path: str | None) -> None:
        self._delegate.active_path = path
        self.viewport().update()
